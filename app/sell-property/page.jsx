"use client"; // ensures this page is never prerendered
...
export default function SellPropertyPage() {
  ...
  // Config helpers
  const addConfig = () => setConfigurations([...configurations, { type: "", carpetArea: "" }]);
  ...
  return (
    <div className="max-w-3xl mx-auto py-16 px-6 space-y-6">
      ...
      <Section title="Seller Contact (Private)">
        <Input value={sellerPhone} set={setSellerPhone} placeholder="Your Phone Number" />
      </Section>
      ...
      <Section title="Price & Possession">
        ...
        <Input value={priceLabel} set={setPriceLabel} placeholder="Price Label (e.g. All inclusive)" />
      ...
      <Section title="Configurations">
        {configurations.map((c, i) => (
          <div key={i} className="flex gap-2">
            <Input value={c.type} set={(v) => updateConfig(i, "type", v)} placeholder="2 BHK" />
            <Input value={c.carpetArea} set={(v) => updateConfig(i, "carpetArea", v)} placeholder="Carpet Area" />
      ...
      <button onClick={handleSubmit} className="bg-black text-white px-6 py-4 rounded-xl w-full">
